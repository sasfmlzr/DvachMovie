...
class DvachUseCaseTest {
    ...
    fun `Deep happy pass`() {
        runBlocking {
            ...
            given(getLinkFilesFromThreadsUseCase.executeAsync(modelOne))
            ...
            given(getLinkFilesFromThreadsUseCase.executeAsync(modelTwo))
            ...
            given(movieConverter.convertFileItemToMovie(files, board, AppConfig.DVACH_URL))
                    .willReturn(resultMovies)
            val resultThreads = listOf(NullThread())
            given(threadConverter.convertFileItemToThread(files, AppConfig.DVACH_URL))
